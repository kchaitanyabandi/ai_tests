...
def process_orders(orders, tax_rate):
    ...
    logger.info("Starting to process %d orders with tax rate %.2f", len(orders), tax_rate)
    ...
    logger.info("Completed processing orders: grand total=%.2f, count=%d", grand_total, len(results))
    return summary

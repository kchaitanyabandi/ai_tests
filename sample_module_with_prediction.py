...
def process_orders(orders, tax_rate):
    ...
    logger.info("Starting to process %d orders with tax_rate %.3f", len(orders), tax_rate)
    results = []
    grand_total = 0.0
    ...
    logger.info("Completed processing orders: %d orders, grand total %.2f", summary["count"], summary["grand_total"])
    return summary
